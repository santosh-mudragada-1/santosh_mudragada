...
const PERSONAS = [
  {
    photo: '/nextrail_casestudy/people/traveler.webp',
    photoPos: '50% 20%',
    name: 'Riya Mehta',
  ...
  {
    photo: '/nextrail_casestudy/people/friends.webp',
    photoPos: '44% 26%',
    name: 'Rajiv Sharma',
...
export function Nextrail() {
  ...
  // The native `autoplay` attribute alone can be unreliable (the other
  // videos on this page all drive play/pause from an effect for the same
  // reason — see Feed2FlyScroll's `Frame`), so this mirrors that: an
  // explicit `.play()` once mounted, skipped entirely under reduced motion.
  useEffect(() => {
  ...
  useGSAP(
    () => {
      ...
      if (!reduced) {
        ...
        // feed2fly reveal device — a small parallax lift. Animates the
        // whole mockup (frame + video together), not the video element by
        // itself — yPercent on just the video shifted its content inside
        // the screen's clip box as you scrolled, looking like the footage
        // was scrolling independently of the phone rather than playing
        // normally while the whole device drifted.
        const dev = q<HTMLElement>(`.${styles.revealDevice}`)[0];
        if (dev) {
          const tw = gsap.fromTo(
            dev,
            { yPercent: 6 },
  ...
  return (
    <article ref={rootRef} className={styles.root}>
      ...
      <section className={styles.reveal} id="feed2fly" aria-labelledby="feed2fly-h">
        ...
        <img className={styles.revealMap} src="/nextrail_casestudy/worldmap.webp" alt="" />
        ...
        <figure className={styles.revealDevice}>
          <DeviceMockup>
            <video
              ...
              src="/nextrail_casestudy/video/nextrail_introduction.webm"
              muted
              loop
              playsInline
            />

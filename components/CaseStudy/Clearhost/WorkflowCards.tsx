...
import { ArrowRight, Check, FileText, GitBranch, Rocket } from './icons';
import { cx } from './cx';
...
function PriorityArtifact({
  ...
  rows: { label: string; impact: number; chip: string }[];
}) {
  return (
    <Sheet>
...
function LinearArtifact({ status, summary }: { status: string; summary: string }) {
  return (
    <Sheet>
...
function QaArtifact({ items }: { items: string[] }) {
  return (
    <Sheet>
...
function LaunchArtifact({ env, release, note }: { env: string; release: string; note: string }) {
  return (
...
function LearnArtifact({ caption, items }: { caption: string; items: string[] }) {
  return (
    <Sheet>
      <Cap>{caption}</Cap>
...
export function ArtifactCard({ step }: { step: WorkflowStep }) {
  switch (step.kind) {
    ...
    case 'linear':
      return <LinearArtifact status={step.status} summary={step.summary} />;
    case 'flow':
    ...
    case 'qa':
      return <QaArtifact items={step.items} />;
    case 'launch':
      return <LaunchArtifact env={step.env} release={step.release} note={step.note} />;
    case 'learn':
      return <LearnArtifact caption={step.caption} items={step.items} />;
    case 'iterate':

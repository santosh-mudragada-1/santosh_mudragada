...
function layoutFor(isDesktop: boolean): Layout {
  ...
  return {
    notes,
    // phones: wide, shallow bands (like the original) — the 6 notes of each
    // group settle as a single evenly-spread row *inside* the band; see
    // packInto() in the timeline, which insets by half a note so nothing
    // spills past the frame. Bands carry ~18px of vertical margin around the
    // row so residual layout drift can't push a note out.
    pms: { x: 3, y: 10, w: 92, h: 16, rot: -0.6, radius: '26px 20px 24px 18px' },
    mid: { x: 6, y: 36, w: 88, h: 15, rot: -1.2, radius: '22px 26px 18px 24px' },
    cm: { x: 3, y: 57, w: 92, h: 16, rot: 0.7, radius: '20px 26px 20px 26px' },
    titles: { pms: { x: 50, y: 4 }, cm: { x: 50, y: 52 } },
    missingLayer: { x: 50, y: 33 },
    annotations: [],
    logo: { x: 50, y: 44 },
    cursorRegions: [
      [
...
export function WorkspaceStory() {
  ...
  useGSAP(
    () => {
      ...
      // Re-measure the stage on every call. The frames are positioned with `%`
      // (top/height), so the browser re-resolves them whenever the stage box
      // changes — 100svh settling after first paint, the mobile address bar
      // hiding, an orientation change. Notes are placed with px transforms, so
      // unless this reads the *current* size they drift above their frames
      // (worse further down the page). Tween end-values that call this must be
      // function-based so ScrollTrigger's invalidateOnRefresh re-runs them.
      const px = (p: Pt) => {
        const r = stage.current?.getBoundingClientRect();
        const w = r?.width || curW || 1;
        const h = r?.height || curH || 1;
        return { x: (p.x / 100) * w, y: (mapY(p.y) / 100) * h };
      };
      const box = () => {
        const r = stage.current!.getBoundingClientRect();
        curW = r.width || 1;
      ...
      mm.add({ isDesktop: '(min-width: 768px)', isMobile: '(max-width: 767.98px)' }, (ctx) => {
        ...
        // phones: shrink the settled notes so a row of 6 fits across a wide
        // band. Applied to every settle/emphasis scale below.
        const settleScale = isDesktop ? 1 : 0.72;
        ...
        // Desktop targets from the generic grid; phones get re-packed below.
        const pmsT = clusterTargets(L.pms, pmsNotes.length, isDesktop ? 2 : 4);
        const cmT = clusterTargets(L.cm, cmNotes.length, isDesktop ? 2 : 4);
        const midT = clusterTargets(L.mid, midNotes.length, 2, true, isDesktop ? 1 : 0.9);
        ...
        // phones: forget the generic clusterTargets math — lay each group out
        // as one evenly-spread row *inside its band*, expressed as fractions of
        // the box inset by half a note (in the pre-map coord space), so the
        // whole note is guaranteed within the rect at any viewport size.
        if (!isDesktop) {
          const noteBase = 76 * settleScale; // .note is 76px below md (see scss)
          const halfX = ((noteBase / 2) / (curW || 1)) * 100;
          const halfY = (((noteBase / 2) / (curH || 1)) * 100 * 100) / (100 - topSafe);
          const packInto = (list: WsNote[], b: Box, cols: number) => {
            const rows = Math.ceil(list.length / cols) || 1;
            const x0 = b.x + halfX;
            const x1 = b.x + b.w - halfX;
            const y0 = b.y + halfY;
            const y1 = b.y + b.h - halfY;
            list.forEach((n, i) => {
              const t = target.get(n.id);
              if (!t) return;
              const fx = cols > 1 ? (i % cols) / (cols - 1) : 0.5;
              const fy = rows > 1 ? Math.floor(i / cols) / (rows - 1) : 0.5;
              t.x = Math.min(
                Math.max(x0 + (x1 - x0) * fx + (((i * 7) % 5) - 2) * 0.5, x0),
                x1,
              );
              t.y = Math.min(
                Math.max(y0 + (y1 - y0) * fy + (((i * 13) % 5) - 2) * 0.4, y0),
                y1,
              );
              t.r = (((i * 29) % 7) - 3) * 1.1;
            });
          };
          // one row of 6 across each wide band
          packInto(pmsNotes, L.pms, pmsNotes.length);
          packInto(cmNotes, L.cm, cmNotes.length);
          packInto(midNotes, L.mid, midNotes.length);
        }
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            ...
            // This pin sits above ProductWorkflow's own pin (refreshPriority: 1)
            // and the footer's curve reveal further down — refresh it first so
            // its spacer height is already settled before either of those
            // recalculates its own position on a ScrollTrigger.refresh().
            refreshPriority: 2,
          },
        });
        ...
        L.notes.forEach((n, i) => {
          ...
          tl.to(
            note(n.id) ?? {},
            {
              autoAlpha: 1,
              scale: 1,
              // function-based so invalidateOnRefresh re-measures the stage
              x: () => px(s).x,
              y: () => px(s).y,
              rotation: s.r ?? 0,
              duration: 0.32,
        ...
        const settleInto = (list: WsNote[], at: number, stagger: number) =>
          list.forEach((n, i) => {
            ...
            tl.to(
              note(n.id) ?? {},
              {
                // function-based: re-resolved on ScrollTrigger.refresh() so the
                // notes track the frames' %-based box when the stage resizes.
                x: () => px(t).x,
                y: () => px(t).y,
                rotation: t.r ?? 0,
                scale: (n.group === 'loose' ? 0.95 : 0.9) * settleScale,
                duration: 0.95,
                ease: 'power3.inOut',
              },
        ...
        midNotes.forEach((n) =>
          tl.to(note(n.id) ?? {}, { scale: 1.04 * settleScale, duration: 0.5 }, 'p3+=0.15'),
        );
